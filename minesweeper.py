...
class MinesweeperEnv:
    ...
    def __init__(self, xdim, ydim, total_mines):
        ...
        self.bomb_penalty = -2  # -(xdim*ydim*2)
        self.open_reward = 0.1  # 1
        self.win_reward = 10  # xdim*ydim
        self.adjacent_reward = 0.2
        self.game_grid, self.player_grid = self.create_grids(
            self.xdim, self.ydim, self.total_mines
        )
                ...
                for i in range(-1, 2):
                    for j in range(-1, 2):
                        if (
                            ...
                            and y + i >= 0
                            and not (i == 0 and j == 0)
                            and not [x + j, y + i] in recursion_list
                        ):
                            tempreward, endstate, recursion_list = self.open(
                                x + j, y + i, recursion_list
                            )
                            reward += tempreward
        ...
        while not her_passed:
            ...
            if training_mode and endstate and not won:
                ...
                if random.random() < HER_chance:
                    self.player_grid = copy.deepcopy(
                        temp_player_grid
                    )  # reset player grid
                    action = self.sample_action()  # choose different action at random
                    while (
                        action in done_actions or action in fail_states
                    ):  # failstates makes sure to not pick the same bomb twice in one turn
    ...
    def print_player_grid_old(self):
        underscores = " "
        underscores += "_ " * len(self.player_grid[0])
        print("mines: ", self.minecount, "safe spots: ", self.safe_spots)
        print(underscores)
...
if __name__ == "__main__":
    ...
    while not endstate:
        ...
        _, reward, endstate, won, _, _ = game.step(action, [], False)
        print(reward)
        game.print_player_grid()
