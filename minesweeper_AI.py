...
class ConvNet(nn.Module):
    def __init__(self, xdim, ydim):
        ...
        self.conv1 = nn.Conv2d(
            1, first_conv_channels, kernel_size=3, stride=1, padding=1
        )
        ...
        self.fc1 = nn.Linear(
            (self.xdim) * (self.ydim) * second_conv_channels, first_fc_out
        )
...
def all_the_checks():
    ...
    if load_check.lower() == "y":
        ...
        with open(
            "./saved_states/total_rewards" + net_type + game_size + ".json", "r"
        ) as file:
            total_rewards = json.load(file)

        with open(
            "./saved_states/moves_per_game" + net_type + game_size + ".json", "r"
        ) as file:
            moves_per_game = json.load(file)
...
def optimize_model(memory):
    ...
    state_action_values = policy_net(state_batch).gather(
        1, action_batch.to(torch.int64).unsqueeze(1)
    )
    ...
    with torch.no_grad():
        next_state_values[non_final_mask] = (
            target_net(non_final_next_states.unsqueeze(1)).max(1).values
        )
...
def play_a_game(fast, notrandom):
    ...
    while True:
        action = select_action(
            state, done_actions, True, not_random=notrandom, steps_done=steps_done
        )
        # let a human play
...
def select_action(state, done_actions, testing_mode, not_random, steps_done):
    ...
    if (sample > eps_threshold or testing_mode) and not_random:
        with torch.no_grad():
            ...
            action_values[0, done_actions] = -float(
                "inf"
            )  # so it doesn't choose a move it already did
...
def save_state(
    ...
    with open(
        "./saved_states/total_rewards" + net_type + game_size + ".json", "w"
    ) as file:
        json.dump(total_rewards, file)
    with open(
        "./saved_states/moves_per_game" + net_type + game_size + ".json", "w"
    ) as file:
        json.dump(moves_per_game, file)


def training_function(
    env, memory, policy_net, target_net, total_rewards, moves_per_game
):
    ...
    while True:
        moves_this_game += 1
        action = select_action(
            state, done_actions, False, not_random=True, steps_done=steps_done
        )
        observation, reward, endstate, won, recursion_list, action = env.step(
            action, done_actions, True
        )
        done_actions.append(int(action))
        done_actions.extend(recursion_list)
    ...
    return env, memory, policy_net, target_net, total_rewards, moves_per_game


def plot_data(data):
    plt.plot(data, marker="o", linestyle="-")  # Plot with dots and lines
    plt.xlabel("Index")
    plt.ylabel("Value")
    plt.grid()
    plt.show()
...
if __name__ == "__main__":
    (
        (xdim, ydim, total_mines),
        (policy_net, target_net, steps_done, total_rewards, moves_per_game),
        (net_type, game_size),
        training,
        num_episodes,
    ) = all_the_checks()
    ...
    if testing:
        ...
        visual_game_amount = 0  # how many games should be showed to the user
        ...
        for i in range(int(len(moves_per_game) / 100)):
            average_moves_per_game.append(
                sum(moves_per_game[i * 100 : (i + 1) * 100]) / 100
            )
        ...
        print("moves per game(avg of 100 ep each): ")
        print(average_moves_per_game)
        print("wins: ", wins, "out of: ", game_amount)
        print("win rate: ", wins / game_amount)

        plot_data(average_moves_per_game)
